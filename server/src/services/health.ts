...
export async function checkKeyHealth(keyId: number): Promise<KeyStatus> {
  ...
  try {
    ...
    try {
      ...
      // Key was encrypted with a different ENCRYPTION_KEY (e.g. dev-fallback key
      // regenerated after DB restore, or plaintext stored in encrypted_key column).
      console.error(
        `[Health] Key ${keyId} (${row.platform}, base=${row.base_url ?? 'default'}) ` +
        `decrypt failed: ${decryptErr.message}. The stored ciphertext may be from a ` +
        `different ENCRYPTION_KEY — re-save this API key to fix.`,
      );
...
// ─── Daily key health check (24h, for auto_key_check setting) ──────────
let cancelDailyKeyCheck: (() => void) | null = null;

export function startDailyKeyCheck(scheduler: Scheduler): void {
  if (cancelDailyKeyCheck) return;
  const INTERVAL = 24 * 60 * 60 * 1000;
  console.log(`[Health] Starting daily key check (every ${INTERVAL / 1000}s)`);
  cancelDailyKeyCheck = scheduler.every(INTERVAL, () =>
    checkAllKeys().catch(err => console.error('[DailyKeyCheck] Failed:', err)),
  );
  // Run once immediately on start
  scheduler.after(5000, () =>
    checkAllKeys().catch(err => console.error('[DailyKeyCheck] Initial check failed:', err)),
  );
}

export function stopDailyKeyCheck(): void {
  if (cancelDailyKeyCheck) {
    cancelDailyKeyCheck();
    cancelDailyKeyCheck = null;
  }
}
