...
import { requireProxyAuth } from './middleware/requireProxyAuth.js';
...
export function createApp(config?: Config) {
  ...
  // Minimal CSP: SPA needs inline scripts (dark-mode theme detection in
  // index.html) and inline styles (React CSS-in-JS / Vite HMR).
  // Explicitly disable 'upgrade-insecure-requests' — helmet adds it by
  // default but this server serves only HTTP on localhost/LAN.
  // HSTS off because this is a single-user local proxy served over HTTP.
  app.use(helmet({
    ...
    hsts: false,
    // Disable X-Frame-Options — CSP frame-ancestors 'self' covers the same
    // threat with better granularity and is supported by all modern browsers.
    frameguard: false,
  }));
  app.use(cors({
  ...
  app.use('/api/fusion', requireAuth, fusionRouter);
  app.use('/api/premium', requireAuth, premiumRouter);
  app.use('/api/smtp', requireAuth, smtpRouter);
  app.use('/api/unified-keys', requireAuth, unifiedKeysRouter);
  ...
  // 统一 API Key + 融合 API Key 认证中间件。在路由之前运行，
  // 将认证信息挂载到 req.proxyAuth 上。
  ...
  // Start periodic expired-session cleanup (every 24 hours) to prevent
  // the sessions table from growing unbounded on low-traffic instances.
  startSessionCleanup();
