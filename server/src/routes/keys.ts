...
const PLATFORMS = [
  ...
  'routeway', 'bazaarlink', 'ainative', 'nara', 'aihorde', 'freeqwq', 'custom',
] as const;
...
const updateKeySchema = z.object({
  ...
  message: '至少提供 enabled 或 label 之一',
});
...
function handleUploadError(err: any, res: Response, next: NextFunction): boolean {
  ...
  if (err.code === 'LIMIT_FILE_SIZE') {
    res.status(413).json({ error: { message: '文件过大，最大支持 5MB' } });
    return true;
  }
  if (err.code === 'LIMIT_FILE_COUNT') {
    res.status(413).json({ error: { message: '文件数量过多，最多 10 个' } });
    return true;
  }
  if (err.message?.includes('Unsupported file type')) {
    res.status(400).json({ error: { message: '不支持的文件类型' } });
    return true;
  }
  next(err);
  return true;
