...
proxyRouter.post('/images/generations', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(httpStatus).json({ error: { message: `image generation error: ${sanitizeProviderErrorMessage(err?.message)}`, type: mediaErrorType(status) } });
  }
});
...
proxyRouter.post('/audio/speech', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const status = err instanceof MediaError ? err.status : 502;
    const httpStatus = status >= 400 && status < 600 ? status : 502;
    res.status(httpStatus).json({ error: { message: `speech error: ${sanitizeProviderErrorMessage(err?.message)}`, type: mediaErrorType(status) } });
  }
});
