...
// 由于 app.ts 中已通过 requireProxyAuth 中间件统一认证（支持统一密钥 + 融合密钥），
// 此处仅检查 req.proxyAuth 是否存在即可，无需重复验证令牌。
function authenticate(req: Request, res: Response): boolean {
  if ((req as any).proxyAuth) return true;
  sendError(res, 401, 'authentication_error', '无效的 API Key');
  return false;
}
