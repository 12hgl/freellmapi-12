...
export type Platform =
  ...
  | 'aihorde'
  // FreeQWQ — OpenAI-compatible free-tier aggregator. Supports both a public
  // anonymous key (built-in) and registered account API keys from freeqwq.com.
  | 'freeqwq'
...
// ---- Fusion API Key Management ----

export interface FusionApiKey {
  id: number;
  name: string;
  keyPrefix: string;
  providers: Platform[];
...
export interface FusionApiKeyCreate {
  name: string;
  providers: Platform[];
  models?: string[];
  rateLimitRpm?: number;
...
export interface FusionApiKeyFull extends FusionApiKey {
  fullKey: string;
}
